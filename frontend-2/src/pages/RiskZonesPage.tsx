        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm flex items-center space-x-4">
          ...
          <div>
            ...
            <div className="text-2xl font-black text-orange-600">
              {riskZones.filter(z => z.category === 'HIGH').length}
            </div>
          </div>
        </div>
      ...
      {riskZones.some(z => z.category === 'CRITICAL') && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start space-x-3 shadow-sm">
          ...
          <div>
            ...
            <p className="text-xs text-red-600 mt-1">{riskZones.filter(z => z.category === 'CRITICAL').map(z => z.name).join(', ')} has exceeded maximum safe deformation thresholds. Evacuation protocols should be on standby.</p>
          </div>
        </div>
      )}
          ...
          <div key={zone.id || idx} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden group">
            <div className={`px-5 py-4 border-b flex justify-between items-center ${getRiskColor(zone.category)} border-opacity-50`}>
              ...
              <div className="text-right">
                <div className="text-2xl font-black">{zone.category === 'CRITICAL' ? 95 : zone.category === 'HIGH' ? 75 : zone.category === 'MODERATE' ? 45 : 15}<span className="text-sm font-medium opacity-70">%</span></div>
            ...
            <div className="p-5">
              <div className="grid grid-cols-2 gap-y-4 gap-x-6 mb-6">
                <div>
                  ...
                  <div className="flex items-end space-x-1">
                    <span className="text-lg font-bold text-slate-800">{zone.maxDeformation !== undefined ? zone.maxDeformation.toFixed(1) : '--'}</span>
              ...
              <div className="mb-4">
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="font-bold text-slate-700">{zone.category === 'CRITICAL' ? 95 : zone.category === 'HIGH' ? 75 : zone.category === 'MODERATE' ? 45 : 15}/100</span>
                </div>
                <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                  <div 
                    className={`h-full rounded-full ${getProgressColor(zone.category)} transition-all duration-1000`} 
                    style={{ width: `${zone.category === 'CRITICAL' ? 95 : zone.category === 'HIGH' ? 75 : zone.category === 'MODERATE' ? 45 : 15}%` }}
                  ></div>
                </div>
              </div>
