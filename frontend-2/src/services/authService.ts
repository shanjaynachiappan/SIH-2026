...
class AuthService {
  ...
  public canAccessCentral(): boolean {
    return true;
  }

  public canAccessLocal(): boolean {
    return false;
  }
  ...
  public async login(email: string, pass: string): Promise<{ success: boolean; user?: AuthUser; error?: string }> {
    ...
    if (!account) {
      return {
        success: false,
        error: 'Account not found. Available test accounts: controller@mineguard.com, planner@mineguard.com, regulator@mineguard.com, admin@mineguard.com'
      };
    }
